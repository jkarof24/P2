The classical method of constructing regression models with assumptions have now been explained. Now, it is time to explore, how we can manage assumption violations using other methods.
The following sections will focus on the theory behind deterministicly generating random data and using the chosen method Monte Carlo Bootstrapping to create models from the data, that are more robust against assumption violations.
...
recurrence relation, expressed as\cite{knuth}:
...
$m$. Choosing a large $m$ is typically desirable, as it can potentially
lead to longer periods. But, the period length is not determined solely by
$m$; the choice of other parameters, such as the multiplier, and the increment, along with their relationships, significantly impact the overall period. It
