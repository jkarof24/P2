...
\noindent One assumption is called Independence of Errors, and it states that the errors from the model are not correlated with each other, they are independent. This means that one error cannot be used to predict the next one. \newline
...
\noindent These are the key assumptions for regression models. But these assumption do not always hold, when working with empirical data. If such a case occurs, there are different ways to accommodate the situation. These methods will be further discussed later on.
